...
class FilesCacheClient(base.BaseCacheClient):
    ...
    def __init__(self, size, basepath,
            ...
            dirmode = 0x1c0, filemode = 0x100, mmap_raw = False, # dirmode = 0o700, filemode = 0o400
            sync_purge = None, value_pickler_protocol = None):
        """
        Params:
        ...
        if checksum_key is not None:
            checksum_key = safeencode(checksum_key)
        ...
        if value_pickler_protocol is None:
            self.value_pickler_protocol = getattr(cPickle, 'DEFAULT_PROTOCOL', 2)
    ...
    def _put(self, key, value, ttl, replace):
        key = safeencode(self.key_pickler(key))
        ...
        try:
            try:
                if hasattr(value, 'fileno') and hasattr(value, 'name') and os.path.isabs(value.name):
                    ...
                    if not reuse_keyfile:
                        ...
                        set_tempfile_delte(keyfile, False)
            ...
            if isinstance(value, (bytes,buffertype)):
                ...
                with self._mktmp() as rawfile:
                    ...
                    set_tempfile_delte(rawfile, False)
                    if not reuse_keyfile:
                        _swap(keyfile.name, keypath, self.size.__iadd__)
                        set_tempfile_delte(keyfile, False)
                ...
                with self._mktmp() as rawfile:
                    self.value_pickler(value, rawfile, self.value_pickler_protocol)
                    rawfile.flush()
